			...
			if ("Modificar".equals(request.getParameter("action"))) {
				...
				if (registroExiste) {
					if (nom.equals("")) {
						...
						int confirma = JOptionPane.showConfirmDialog(null, "Â¿Desea actualizar la informaciÃ³n de este perfil?");
					...
					request.getRequestDispatcher("./Asignaturas.jsp").forward(request, response);
					JOptionPane.showMessageDialog(null, "Registro inexistente, por favor verifique la identificaciÃ³n del perfil", "Advertencia - AsesoriasTyT", JOptionPane.WARNING_MESSAGE);
				}
			}
