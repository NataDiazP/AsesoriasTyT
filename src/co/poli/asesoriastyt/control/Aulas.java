				...
				try {
					...
					while (r.next()) {
						if (id.equals(r.getString(1))) {
							JOptionPane.showMessageDialog(null, "Este registro ya existe, por favor verifique la identificaciÃ³n del bloque", "Advertencia - AsesoriasTyT",
			...
			if ("Consultar".equals(request.getParameter("action"))) {
				...
				try {
					...
					while (r1.next()) {
						if (!id.equals(r1.getString(1)) && (!id.equals(""))) {
							response.sendRedirect("Aulas.jsp");
							JOptionPane.showMessageDialog(null, "Registro inexistente, por favor verifique la identificaciÃ³n del bloque", "Advertencia - AsesoriasTyT", JOptionPane.WARNING_MESSAGE);
						}
					}
				} catch (SQLException e) {
					e.printStackTrace();
